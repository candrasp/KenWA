...
const Blast = {
  ...
  async renderTemplatesTab(container) {
    ...
    await Table.create({
      ...
      columns: [
        ...
        { label: 'Pesan', key: 'body', render: (v) => `<div class="text-truncate" style="max-width: 400px;">${v || ''}</div>` },
        {
          ...
          render: (id) => `
            <div class="flex gap-2">
              <button class="action-btn use-template" title="Gunakan" data-id="${id}">
                <i class='bx bx-send'></i>
              </button>
              <button class="action-btn edit-template" title="Edit" data-id="${id}">
                <i class='bx bx-edit-alt'></i>
              </button>
              <button class="action-btn delete-template" title="Hapus" data-id="${id}">
    ...
    tableEl.addEventListener('click', (e) => {
      const useBtn = e.target.closest('.action-btn.use-template');
      const editBtn = e.target.closest('.action-btn.edit-template');
      const deleteBtn = e.target.closest('.action-btn.delete-template');

      if (useBtn) this.useTemplate(useBtn.dataset.id);
      if (editBtn) this.openEditTemplateModal(editBtn.dataset.id);
      if (deleteBtn) {
