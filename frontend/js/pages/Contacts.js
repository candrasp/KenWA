...
const Contacts = {
        ...
        },
        {
          ...
          render: (id) => `
            <div class="flex gap-2">
              <button class="action-btn edit" title="Edit" data-id="${id}">
                <i class='bx bx-edit-alt'></i>
              </button>
              <button class="action-btn delete" title="Hapus" data-id="${id}">
  ...
  async renderTagsTab(container) {
    ...
    await Table.create({
      ...
      columns: [
        ...
        { label: 'Jumlah Kontak', key: 'contact_count', sortable: true, render: (v) => `<span class="badge" style="background: rgba(255,255,255,0.05); color: var(--text-main);">${v || 0}</span>` },
        {
          ...
          render: (id) => `
            <div class="flex gap-2">
              <button class="action-btn edit-tag" title="Edit" data-id="${id}">
                <i class='bx bx-edit-alt'></i>
              </button>
              <button class="action-btn delete-tag" title="Hapus" data-id="${id}">
                <i class='bx bx-trash'></i>
              </button>
            </div>
          `
    ...
    tableEl.addEventListener('click', (e) => {
      const editBtn = e.target.closest('.action-btn.edit-tag');
      const deleteBtn = e.target.closest('.action-btn.delete-tag');

      if (editBtn) {
        this.openEditTagModal(editBtn.dataset.id);
      }
      ...
      onConfirm: async () => {
        ...
        reader.onload = async (e) => {
          ...
          for (const line of lines) {
            ...
            // Skip jika nomor kosong setelah dibersihkan (misal: baris header "Nama, Nomor")
            if (!phone || phone.length < 5) {
              console.warn('[Import] Skipping invalid row:', name, phoneRaw);
              failed++;
              continue;
            }
