...
const Settings = {
  ...
  async checkAppVersion() {
    // 1. Coba ambil dari config global (Backend)
    if (window.appConfig?.version) {
      document.getElementById('current-version').textContent = `v${window.appConfig.version}`;
      this.updateInfo.currentVersion = window.appConfig.version;
    }

    // 2. Coba ambil dari Tauri sebagai verifikasi
