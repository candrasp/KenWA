...
async function request(method, path, body, retries = 5) {
  const opts = {
    method,
  ...
  try {
    ...
    // Jika koneksi ditolak atau gagal fetch (server mungkin sedang booting), coba lagi
    if (retries > 0 && (err.name === 'TypeError' || err.message.includes('Failed to fetch'))) {
      console.warn(`[API] Connection failed. Retrying... (${retries} left)`);
...
export const WaAPI = {
  ...
  fetchQR:    ()       => request('GET',  '/wa/qr'), // Polling QR dari cache backend
